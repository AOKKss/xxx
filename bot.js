...
client.on("message", message => {
    ...
    if(command === "give") {
      if(!message.author.id === message.guild.owner) return message.reply("لا تستطيع يجب توفر علي خاصيت!");
      const user = message.mentions.users.first() || client.users.get(args[0]);
      if(!user) return message.reply("You must mention someone or give their ID!");
      const pointsToAdd = parseInt(args[1], 10);
      if(!pointsToAdd) return message.reply("كم تريد اعطاء...");
...
client.on('message', message => {
  ...
  if (command == "ban") {
               if(!message.channel.guild) return message.reply('** هذه خاصيت اداره فقط**');
         
  if(!message.guild.member(message.author).hasPermission("BAN_MEMBERS")) return message.reply("**يجب توفر خاصيت ` BAN_MEMBERS ` Permission**");
  if(!message.guild.member(client.user).hasPermission("BAN_MEMBERS")) return message.reply("**يجب توفر خاصيت ` BAN_MEMBERS ` Permission**");
  ...
  .bannable) return message.reply(`متحصل علي رتبه مسبقا !`);
...
client.on('message', message => {
  ...
  if (command == "kick") {
               if(!message.channel.guild) return message.reply('**هذا امر فقط لسرفر**');
         
  if(!message.guild.member(message.author).hasPermission("KICK_MEMBERS")) return message.reply("**ليس عندي صلاحية ` KICK_MEMBERS`**");
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("** ليس عندي صلاحية` KICK_MEMBERS`**");
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  ...
  if (!message.guild.member(user)
  .kickable) return message.reply("**هذا مستخدم متحصل علي رتبه**");
...
client.on('message', message => {
         if(message.content === prefix + "closeroom") {
                             if(!message.channel.guild) return message.reply('**هذه صلاحية فقط لسرفر**');
...
client.on('message', message => {
  if(message.content.includes('discord.gg')){
                                          if(!message.channel.guild) return message.reply('** جرب خاص ? :thinking:   **');
      if (!message.member.hasPermissions(['ADMINISTRATOR'])){
      message.delete()
  return message.reply(`** لا تستطيع هنا :angry: ! **`)
  }
}
});
client.on('message', message => {
...
.setColor("#9B59B6")
.addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/CNxHEGY**")
...
if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply(`**ليست لديك صلاحيات المطلوبا !`);
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES_OR_PERMISSIONS")) return message.reply(`**ليست لديك صلاحية مطلوبا !`);
...
client.on('message', message => {
    ...
    .setURL(`https://discordapp.com/api/oauth2/authorize?client_id517633091280306176=bot id&permissions=8&scope=bot`)  // حط اي دي بوتك
...
client.login("NTE3NjMzMDkxMjgwMzA2MTc2.DuFFDg.etk2qmtNTO0J9ElvpuhIU3_ssO4");
